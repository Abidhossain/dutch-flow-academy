...
class EVORM_Cron{
	...
	function perform_cron($event_id, $field_variable){
		...
		// if event is cancelled stop reminders
	...
	function schedule_reminders($event_id, $field_variable){		
		...
		if($cron_timestamp == false){
			...
			$check_enable = EVO()->cal->check_yn( $var, 'evcal_'.$addon ); 			
			$check_time = EVO()->cal->get_prop( $var.'_time', 'evcal_'.$addon );
			$check_msg = EVO()->cal->get_prop( $var.'_message','evcal_'. $addon ); 
	...
	function get_reminder_time($event_id, $varAR){
		...
		$reminder_offset_time = EVO()->cal->get_prop( $var.'_time' , 'evcal_'.$addon );
