<?php
/**
 * Notification Controller: Assignment Submitted
 *
 ...
 * @version  1.0.0-beta.6
 */
class LLMS_Assignments_Notification_Controller_Assignment_Submitted extends LLMS_Abstract_Notification_Controller {
	 ...
	 * @var  [type]
	 */
	public $id = 'assignment_submitted';
	...
	protected function get_subscriber( $subscriber ) {
		...
		if ( 'author' === $subscriber ) {
			...
			if ( $sub->exists() ) {
				$assignment = $sub->get_assignment();
				if ( $assignment ) {
					$course = $assignment->get_course();
					if ( $course ) {
						$uid = $course->get( 'author' );
					}
				}
			}
		}
	...
	public function get_title() {
		return __( 'Assignments: Assignment Submitted', 'lifterlms-assignments' );
	}
...
return LLMS_Assignments_Notification_Controller_Assignment_Submitted::instance();
