...
class LLMS_WC_Access_Plans {
	...
	public function output_js() {
		...
		<script>( function( $ ){
			...
			// When "is free" is selected, toggle the visibility of the plan availability options and membership settings.
			// Free items use availability from here, product connections use availability of the product/variation in WC settings.
			$( document ).on( 'change', 'select[name^="_llms_plans"][name*="is_free"]', function() {
				var $is_free = $( this ),
					$plan = $is_free.closest( '.llms-access-plan' ),
					$price = $plan.find( 'input.llms-plan-price' ),
					$availability = $plan.find( 'select[name^="_llms_plans"][name*="availability"]').closest( '.d-1of3' ),
					$availability_restrictions = $plan.find( 'select[name^="_llms_plans"][name*="availability_restrictions"]').closest( '.d-1of2' ),
					$redirects = $plan.find( '.llms-redirection-settings' ),
					$membership_settings_heading = $plan.find( '.llms-plan-row-6' ).prev( 'h4' );

				// Check if the select dropdown has value free or not.
				if ( $is_free.val() === 'yes' ) {
					$availability.show();
					$availability_restrictions.show();
					$membership_settings_heading.show();
					$redirects.show();
				} else {
					$availability.hide();
					$availability_restrictions.hide();
					$membership_settings_heading.hide();
					$redirects.hide();
					$price.val( 1 ); // Set a price to prevent validation issues.
					// Set the availability to option with value "open" to avoid validation issues.
					$availability.find( 'option[value="open"]' ).prop( 'selected', true );
				}
			} );
	...
	public function output_fields( $plan, $id, $order ) {
		...
		<div class="llms-plan-row-wc" data-controller="llms-is-free" data-value-is-not="yes">
			...
			<p class="llms-notice llms-notice-info"><?php printf( esc_html__( 'You can restrict what memberships can purchase this access plan in the %1$sWooCommerce product settings%2$s.', 'lifterlms-woocommerce' ), '<a href="https://lifterlms.com/docs/getting-started-with-lifterlms-and-woocommerce/#members-only" target="_blank">', '</a>' ); ?></p>
