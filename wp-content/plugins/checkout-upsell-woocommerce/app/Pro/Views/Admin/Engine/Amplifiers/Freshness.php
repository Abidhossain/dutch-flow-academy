...
<div class="flex-fill p-3 cuw-amplifier-wrapper">
    <select class="form-control" name="engine_amplifiers[<?php echo esc_attr($key); ?>][operator]">
        <option value="desc" <?php if ($operator == 'desc') echo "selected"; ?>><?php esc_html_e("New to Old", 'checkout-upsell-woocommerce'); ?></option>
        <option value="asc" <?php if ($operator == 'asc') echo "selected"; ?>><?php esc_html_e("Old to New", 'checkout-upsell-woocommerce'); ?></option>
    </select>
</div>
