...
$order_product_ids = isset($condition['order_product_ids']) && !empty($condition['order_product_ids']) ? array_flip($condition['order_product_ids']) : [];
...
$list_status = CUW()->wc->getOrderStatuses();
foreach ($order_statuses as $slug => $status) {
    if (isset($list_status[$slug])) {
        $order_status[$slug] = $list_status[$slug];
    }
}
foreach ($order_product_ids as $id => $index) {
    $order_product_ids[$id] = CUW()->wc->getProductTitle($id, true);
}
?>

<div class="condition-values">
    <select multiple class="select2-list" name="conditions[<?php echo esc_attr($key); ?>][order_product_ids][]"
            data-list="products"
            data-placeholder=" <?php esc_html_e("Choose products", 'checkout-upsell-woocommerce'); ?>">
        <?php foreach ($order_product_ids as $id => $name) { ?>
...
<div class="condition-values">
    <select multiple class="select2-local" name="conditions[<?php echo esc_attr($key); ?>][order_statuses][]"
            data-placeholder=" <?php esc_html_e("Choose statuses", 'checkout-upsell-woocommerce'); ?>">
        <?php foreach ($list_status as $slug => $name) { ?>
