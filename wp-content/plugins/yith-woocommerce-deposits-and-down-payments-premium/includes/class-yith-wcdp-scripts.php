...
if ( ! class_exists( 'YITH_WCDP_Scripts' ) ) {
	/**
	 * Scripts utility
	 *
	 * @since 1.0.0
	 */
	class YITH_WCDP_Scripts {
		...
		protected static function translate( $handle ) {
			wp_set_script_translations( $handle, 'yith-woocommerce-deposits-and-down-payments', YITH_WCDP_DIR . 'languages' );
		}
		...
		public static function get_relative_path( $handle, $section = '' ) {
			...
			return apply_filters( 'yith_wcdp_assets_path', "assets/js/{$subpath}{$filename}{$extension}", $handle, $section );
		}
		...
		public static function get_url( $handle, $section = '' ) {
			...
			return apply_filters( 'yith_wcdp_assets_url', YITH_WCDP_URL . $path, $handle, $section );
		}
